...
//	theSimulation->allSurfaces->theSurface->printCurvatureDistribution( theSimulation->allSurfaces->r );
	...
	///// BEGIN HELIX MOD
	{
		// scale the lattice so that two triangles have area 1/4 nm^2.
		double target_area = 100.0; // 1  nm^2
		double inclusion_c0 = block.util_value2;
		int start_f = theSurface1->nf_faces/2;
		int start_t = theSurface1->theFormulas[start_f*theSurface1->nf_g_q_p+0].tri;
		
		int tri_1_t_set[4] = { start_t, 
				theSurface1->theTriangles[start_t].border_tri[0], 
				theSurface1->theTriangles[start_t].border_tri[1], 
				theSurface1->theTriangles[start_t].border_tri[2] };
		int tri_1_f_set[4] = { 
			theSurface1->theTriangles[tri_1_t_set[0]].f,
			theSurface1->theTriangles[tri_1_t_set[1]].f,
			theSurface1->theTriangles[tri_1_t_set[2]].f,
			theSurface1->theTriangles[tri_1_t_set[3]].f };
	
		
		double inclusion_area = theSurface1->g(tri_1_f_set[0],1.0/3.0,1.0/3.0, r1) * 0.5 + 
					theSurface1->g(tri_1_f_set[1],1.0/3.0,1.0/3.0, r1) * 0.5 +
					theSurface1->g(tri_1_f_set[2],1.0/3.0,1.0/3.0, r1) * 0.5 +
					theSurface1->g(tri_1_f_set[3],1.0/3.0,1.0/3.0, r1) * 0.5;
	
		double scale = sqrt(target_area/inclusion_area);
	
		for( int ix = 0; ix < theSurface1->nv; ix++ )
		{
			r1[3*ix+0] *= scale;
			r1[3*ix+1] *= scale;
			r1[3*ix+2] *= scale;
		...
		theSurface1->PBC_vec[0][0] *= scale;
		theSurface1->PBC_vec[1][1] *= scale;
		theSurface1->PBC_vec[2][2] *= scale;
	
		theSimulation->PBC_vec[0][0] *= scale;
		theSimulation->PBC_vec[1][1] *= scale;
		theSimulation->PBC_vec[2][2] *= scale;
		...
		double helix_helix_dist = block.util_value;
	
		// go in one direction this distance until we hit approx what we want.
		
		int f_1 = tri_1_f_set[0], nf = tri_1_f_set[0];
		double uv1[2] = { 1.0/3.0, 1.0/3.0 };
	
		double drdu[3];
		double drdv[3];
		
		theSurface1->ru( f_1, uv1[0], uv1[1], r1, drdu);
	
		double l_map = sqrt(drdu[0]*drdu[0]+drdu[1]*drdu[1]+drdu[2]*drdu[2]);
		
		double duv1[2] = { helix_helix_dist/l_map, 0.0 }; 
		
		do {
			f_1 = nf;
			nf = theSurface1->nextFace( f_1, uv1+0, uv1+1, duv1+0, duv1+1, r1 ); 
		} while( nf != f_1 );
		
		int tri_target;
		if( nf < theSurface1->nf_faces )
			tri_target = theSurface1->theFormulas[nf*theSurface1->nf_g_q_p].tri;
		else
			tri_target = theSurface1->theIrregularFormulas[(nf-theSurface1->nf_faces)*theSurface1->nf_irr_pts].tri;
	
		int tri_2_t_set[4] = { tri_target, 
						theSurface1->theTriangles[tri_target].border_tri[0],	
						theSurface1->theTriangles[tri_target].border_tri[1],	
						theSurface1->theTriangles[tri_target].border_tri[2] };	
		int tri_2_f_set[4] = { theSurface1->theTriangles[tri_2_t_set[0]].f, 
					theSurface1->theTriangles[tri_2_t_set[1]].f,
					theSurface1->theTriangles[tri_2_t_set[2]].f,
					theSurface1->theTriangles[tri_2_t_set[3]].f };
	
		double inclusion_area_1 = theSurface1->g(tri_1_f_set[0],1.0/3.0,1.0/3.0, r1) * 0.5 + 
					theSurface1->g(tri_1_f_set[1],1.0/3.0,1.0/3.0, r1) * 0.5 +
					theSurface1->g(tri_1_f_set[2],1.0/3.0,1.0/3.0, r1) * 0.5 +
					theSurface1->g(tri_1_f_set[3],1.0/3.0,1.0/3.0, r1) * 0.5;
		
		double inclusion_area_2 = theSurface1->g(tri_2_f_set[0],1.0/3.0,1.0/3.0, r1) * 0.5 + 
					theSurface1->g(tri_2_f_set[1],1.0/3.0,1.0/3.0, r1) * 0.5 +
					theSurface1->g(tri_2_f_set[2],1.0/3.0,1.0/3.0, r1) * 0.5 +
					theSurface1->g(tri_2_f_set[3],1.0/3.0,1.0/3.0, r1) * 0.5;
		
		int clash = 0;
		for( int f = 0; f < 4; f++ )
		for( int f2 = 0; f2 < 4; f2++ )
		{
			if( tri_1_f_set[f] == tri_2_f_set[f2] ) 
			{
				clash = 1;
			}
		}	
		if( clash ) printf("CLASH.\n");
		printf("nfaces: %d nreg: %d f1: %d %d %d %d f2: %d %d %d %d\n", theSurface1->nt, theSurface1->nf_faces,
			tri_1_f_set[0],
			tri_1_f_set[1],
			tri_1_f_set[2],
			tri_1_f_set[3],
			...
			tri_2_f_set[2],
			tri_2_f_set[3] );
		printf("inclusion area 1: %lf area 2: %lf\n", inclusion_area_1, inclusion_area_2 );
	
		double inc1[3],nrm1[3], inc2[3],nrm2[3];
		theSurface1->evaluateRNRM( tri_1_f_set[0], 1.0/3.0, 1.0/3.0, inc1, nrm1, r1 );
		theSurface1->evaluateRNRM( tri_2_f_set[0], 1.0/3.0, 1.0/3.0, inc2, nrm2, r1 );
	
		double dr[3] = { inc1[0] -inc2[0], inc1[1]-inc2[1], inc1[2]-inc2[2] };

		theSimulation->wrapPBC( dr, theSimulation->alpha );
		double len = normalize(dr);
	
		printf("Gopher tunnels distance %lf\n", len );
	
		double area_tot, area0;
		theSurface1->area( r1, -1, &area_tot, &area0);

		double R = sqrt( area_tot / (4*M_PI) );
	
		printf("Radius: %lf\n", R);

		// set the spontaneous curvature of the inclusion.

		int *f_sets[] = { tri_1_f_set, tri_2_f_set };
		...
		for( int set = 0; set < 2; set++ )
		{
			for( int fx = 0; fx < 4; fx++ )	
			{
				...
				if( f < theSurface1->nf_faces )
				{
					for( int t = 0; t < theSurface1->nf_g_q_p; t++ )
						theSurface1->theFormulas[f*theSurface1->nf_g_q_p+t].c0 = inclusion_c0;
				} 
				else {
					int irr_f = f - theSurface1->nf_faces;
					for( int t = 0; t < theSurface1->nf_irr_pts; t++ )
						theSurface1->theIrregularFormulas[irr_f*theSurface1->nf_irr_pts+t].c0 = inclusion_c0;
				} 
			}
		}		

		double fen_test = theSurface1->fenergy( tri_1_f_set[0], r1, NULL ); 
		printf("fen_test: %le\n", fen_test );
	}
	///// END HELIX MOD
...
//end
	...
	if( block.nmin > 0 )
	{
		...
		{
			theSimulation->minimize(any_do_gen_q,0);
			for( surface_record *sRec = theSimulation->allSurfaces; sRec; sRec = sRec->next )
				sRec->theSurface->put(sRec->r);
			for( int c = 0; c < theSimulation->ncomplex; c++ ) theSimulation->allComplexes[c]->refresh(theSimulation);
			if( par_info.my_id == BASE_TASK )
